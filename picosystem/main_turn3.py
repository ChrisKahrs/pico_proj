...
def update(tick):
    ...
    if g.page ==3:
        if len(g.players) > 0:
            g.players[g.current_player].update_time(1)
    # if paused stop timer, or reset it to new time after pause if over?
#         delta = time.ticks_diff(g.start_time, time.ticks_us())
#         if delta > 1000000 or (delta < -1000000):
#             new_text = str(math.floor((time.ticks_us() - original_time)/1000000))
#             print("gc free ", gc.mem_free(), " delta_time: ", new_text)
#         g.start_time = time.ticks_us()
    ...
    if g.page == 3:    
        for i in g.players:
            ...
            if i.pen_white:
                ...
                pen(0,0,0)
            text(str(i.time_counter), 20, y_down+4)
